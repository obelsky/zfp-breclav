...
export const metadata: Metadata = {
  title: 'Pojištění | Finanční poradna | ZFP GROUP Břeclav',
  description: 'Jak se správně pojistit a neplatit zbytečně. Rady od certifikovaných poradců.',
};

const CATEGORY_SLUG = 'pojisteni';
const CATEGORY_TITLE = 'Pojištění';
const CATEGORY_DESC = 'Jak se správně pojistit a neplatit zbytečně';
    ...
    <section className="pt-24 lg:pt-32 pb-20 min-h-screen">
      <div className="container-custom">
        ...
        <div className="mb-16">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-blue-400 mb-6">{CATEGORY_TITLE}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {articles.map((article) => (
                <Link key={article.id} href={`/poradna/${article.slug}`}
                  className="group bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-blue-400/30 transition-all">
                  ...
                  <div className="p-6">
                    <h3 className="text-xl font-semibold mb-2 group-hover:text-blue-400 transition-colors">{article.title}</h3>
        ...
        <div className="bg-gradient-to-br from-blue-500/10 to-zfp-dark border border-blue-500/20 rounded-2xl p-12 text-center">
          <h2 className="text-3xl mb-6">Potřebujete osobní radu?</h2>
          <p className="text-xl text-white/70 mb-8 max-w-2xl mx-auto">Spojte se s námi a probereme vaše pojištění</p>
          <Link href="/kontakt" className="inline-block px-10 py-5 bg-blue-500 hover:bg-blue-600 text-white font-medium tracking-wider uppercase rounded-lg transition-all">
            Kontaktovat poradce
          </Link>
        </div>
      </div>
