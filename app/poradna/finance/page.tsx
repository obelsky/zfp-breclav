...
export const metadata: Metadata = {
  title: 'Finance | Finanční poradna | ZFP GROUP Břeclav',
  description: 'Praktické rady o rozpočtu, spoření a finančním plánování od certifikovaných poradců.',
};

const CATEGORY_SLUG = 'finance';
const CATEGORY_COLOR = '#D4A853';
const CATEGORY_TITLE = 'Finance';
const CATEGORY_DESC = 'Praktické rady o rozpočtu, spoření a finančním plánování';
    ...
    <section className="pt-24 lg:pt-32 pb-20 min-h-screen">
      <div className="container-custom">
        ...
        <div className="mb-16">
          ...
          <h1 className="text-4xl md:text-5xl font-bold text-zfp-gold mb-6">{CATEGORY_TITLE}</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              {articles.map((article) => (
                <Link key={article.id} href={`/poradna/${article.slug}`}
                  className="group bg-white/5 border border-white/10 rounded-xl overflow-hidden hover:border-zfp-gold/30 transition-all">
        ...
        <div className="bg-gradient-to-br from-zfp-gold/10 to-zfp-dark border border-zfp-gold/20 rounded-2xl p-12 text-center">
          <h2 className="text-3xl mb-6">Potřebujete osobní radu?</h2>
          <p className="text-xl text-white/70 mb-8 max-w-2xl mx-auto">Spojte se s námi a probereme vaši finanční situaci</p>
          <Link href="/kontakt" className="inline-block px-10 py-5 bg-zfp-gold hover:bg-zfp-gold/90 text-white font-medium tracking-wider uppercase rounded-lg transition-all">
            Kontaktovat poradce
          </Link>
        </div>
      </div>
