...
export default function Dashboard() {
  ...
  const loadData = async () => {
    ...
    const statsData = await getLeadStats(advisorId);
    setStats(statsData);
    
    const leads = await getLeads(advisorId);
    const recent = leads
          ...
          {recentLeads.length === 0 ? (
            <div className="text-center py-12 text-white/40">
              ...
              <p className="text-lg mb-2">
                {user?.role === 'admin' ? 'Zatím žádné poptávky' : 'Nemáte přidělené žádné poptávky'}
              </p>
              <p className="text-sm">
                {user?.role === 'admin' 
                  ? 'Poptávky se zobrazí jakmile je klient odešle přes formulář nebo kalkulačku'
                  : 'Poptávky vám přidělí administrátor systému'}
              </p>
            </div>
          ) : (
