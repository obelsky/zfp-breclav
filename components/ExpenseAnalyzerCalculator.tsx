...
interface ExpenseAnalyzerCalculatorProps {
  onContactClick?: (data?: Record<string, any>) => void;
}

export default function ExpenseAnalyzerCalculator({ onContactClick }: ExpenseAnalyzerCalculatorProps) {
  ...
  const expenses = [
    { name: 'Bydlení', amount: mortgage },
    { name: 'Energie', amount: utilities },
    { name: 'Pojištění', amount: insurance },
    { name: 'Auto úvěr', amount: carLoan },
    { name: 'Potraviny', amount: groceries },
    { name: 'Restaurace', amount: restaurants },
    { name: 'Doprava', amount: transport },
    { name: 'Zábava', amount: entertainment },
    { name: 'Shopping', amount: shopping },
    { name: 'Ostatní', amount: other },
  ...
  const getInsights = () => {
    ...
    if (restaurants > groceries * 0.5) {
      ...
      insights.push({
        ...
        text: `Snížením o 30% ušetříte ${formatCurrency(saving)}/měsíc.`,
        savings: saving * 12
      });
    }
    ...
    if (shopping > monthlyIncome * 0.1) {
      ...
      insights.push({
        ...
        text: `Ušetřete ${formatCurrency(saving)}/měsíc.`,
        savings: saving * 12
      });
    }
    ...
    if (remaining < monthlyIncome * 0.2) {
      insights.push({
        ...
        text: `Šetříte pouze ${remainingPercent.toFixed(0)}%. Doporučujeme alespoň 20%.`,
        savings: 0
      });
    }
    ...
    if (remaining > monthlyIncome * 0.3) {
      insights.push({
        ...
        text: `Šetříte ${remainingPercent.toFixed(0)}% příjmu. Skvělé!`,
        savings: 0
      });
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-8">
            ...
            <div className="bg-green-500/10 border border-green-500/20 rounded-xl p-6">
              ...
              <div className="text-3xl font-bold text-green-400 mb-3">{formatCurrency(monthlyIncome)}</div>
              <input type="range" min="20000" max="150000" step="5000" value={monthlyIncome} onChange={(e) => setMonthlyIncome(Number(e.target.value))} className="w-full h-2 bg-white/10 rounded-lg appearance-none cursor-pointer slider" />
            </div>
            ...
            <div>
              ...
              <div className="space-y-4">
                {[
                  { label: 'Bydlení', value: mortgage, setter: setMortgage },
                  { label: 'Energie', value: utilities, setter: setUtilities },
                  { label: 'Pojištění', value: insurance, setter: setInsurance },
                  { label: 'Auto úvěr', value: carLoan, setter: setCarLoan },
                ].map((item, i) => (
                  <div key={i} className="bg-white/5 rounded-lg p-4">
                    ...
                    <input type="range" min="0" max="30000" step="500" value={item.value} onChange={(e) => item.setter(Number(e.target.value))} className="w-full h-2 bg-white/10 rounded-lg appearance-none cursor-pointer slider" />
                  </div>
                ))}
              </div>
            </div>
              ...
              <div className="space-y-4">
                {[
                  ...
                  <div key={i} className="bg-white/5 rounded-lg p-4">
                    ...
                    <input type="range" min="0" max="20000" step="500" value={item.value} onChange={(e) => item.setter(Number(e.target.value))} className="w-full h-2 bg-white/10 rounded-lg appearance-none cursor-pointer slider" />
                  </div>
                ))}
              </div>
            </div>
            ...
            >
              ...
              <div className="space-y-3">
                ...
                <div className="pt-3 border-t border-white/10">
                  ...
                  <div className="text-3xl font-bold text-zfp-gold">{remainingPercent.toFixed(0)}%</div>
                </div>
              </div>
            </motion.div>
                ...
                {expenses.filter(e => e.amount > 0).slice(0, 5).map((expense, i) => {
                  ...
                  return (
                    <div key={i}>
                      ...
                      <div className="text-xs text-white/50 text-right mt-1">{percent.toFixed(0)}%</div>
                    </div>
                  );
                })}
              </div>
              ...
              >
                ...
                {totalPotentialSavings > 0 && (
                  <div className="mt-4 p-4 bg-green-500/20 border-2 border-green-500/40 rounded-xl text-center">
                    ...
                    <div className="text-3xl font-bold text-green-400">{formatCurrency(totalPotentialSavings)}/rok</div>
                  </div>
                )}
              </motion.div>
            )}
